...
1. **增量更新机制**
    - 区块高度指针记录 ✅
    - 获取合约创建区块高度 ✅ (419912164)
    - 分批次获取历史数据 ✅
    - 修复区块范围计算bug ✅ (fromBlock + 1 避免重复)
    - 添加时间戳验证 ✅
    - 成功获取所有历史数据 ✅ (3个主题)
...
{
  "topicId": "3",           // 字符串
  "transactionHash": "0x2476...",
  "timestamp": "1768123926" // Unix时间戳字符串
}
```
...
{
  "topicId": 2n,            // BigInt
  "hash": "0x90211...",     // 不是transactionHash
  "timestamp": "2026/1/11 14:07:19" // 格式化时间字符串
}
```

#### 2. 字段差异
- 钱包数据多了 `replyCount` 字段
- `hash` vs `transactionHash` 字段名不一致
- 数据类型不一致（字符串 vs BigInt vs 格式化时间）

### 需要修复的问题列表
1. **统一数据源字段名** - 确定使用 `hash` 还是 `transactionHash`
2. **统一数据类型** - topicId、timestamp 等字段类型统一
3. **replyCount 渐进加载** - 基础数据 + 额外RPC调用的合理时机
4. **恢复 range limit** - 临时修改的区块范围需要改回兼容Phantom

### 🚨 严重问题：区块范围计算错误

**问题分析**：
Arbitrum 出块时间约为 0.25 秒/块，9000 个区块计算：
- 总秒数：9000 块 × 0.25 秒/块 = 2250 秒
- 换算分钟：2250 秒 ÷ 60 = 37.5 分钟
- 换算小时：37.5 分钟 ÷ 60 = 0.625 小时

**结论**：9000 个区块 ≈ 37.5 分钟

**影响**：
- GitHub Actions 每小时运行一次
- 钱包用户只能看到 37.5 分钟内的帖子
- 严重缺失历史数据，用户体验极差

**根本原因**：
误将 Ethereum 出块时间（~12秒）套用到 Arbitrum

✅ **已解决**：
- 修改 GitHub Actions 为每半小时运行一次：`'5,35 * * * *'`
- 9000 区块（37.5 分钟）现在可以完整覆盖更新周期

✅ **数据结构统一**：
- 统一字段名：钱包 `hash` → `transactionHash`
- 统一数据类型：topicId/replyId → number，timestamp/blockNumber → string
- 统一排序规则：topic 逆序（v2ex风格），reply 顺序
- 修复去重逻辑：基于 `transactionHash` 去重
- 修复 Svelte each 块 key 引用：`topic.transactionHash`
...
**解决方案**：
1. ✅ 调整 GitHub Actions 运行频率
2. ✅ 统一缓存和钱包数据格式
3. ✅ 实现正确的排序和去重逻辑

### 代码逻辑审查
需要检查数据获取脚本和前端处理逻辑，确定是谁改变了原始字段名。
